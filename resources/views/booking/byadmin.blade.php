	...
	.controller('BookingController', ['$scope' ,'$http' , function($scope , $http){
		...
		$scope.booking = function(){
			...
			if($scope.list.item != null){
				$http.post('/booking/create/admin',$scope.input).success(function(d){
					console.log(d);
					if(d.result){
						window.location = '/summary/admin/'+d.bookingCode;
		...
		$scope.search = function(){
			...
			$http.post('/booking/search/admin' , $scope.input).success(function(d){
				//console.log(d);
				if(d.result){
					$scope.ui.panalPrice = true;
