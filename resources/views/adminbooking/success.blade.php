...
@section('content')
        ...
        <div class="summary" ng-controller="SummaryController">
            <div class="row">
                <h4 class="text-right">ณ วันที่ {{date('d/m')}}/{{ (date('Y') +543) }} {{ date('H:i:s') }}</h4>
                <h4 class="text-right">รหัสการจอง {{  $booking_code  }}</h4>
                <h3>ชื่อ : {{ session()->get('member')->name }} </h3>
                ...
                <div class="col-sm-8 col-sm-offset-2 box-booking-button">
                    <a class="btn btn-jj btn-block" href="javascript:void(0)" ng-click="destroy()" >กลับสู้หน้าหลัก</a>
                </div>
            </div>
    ...
    <script type="text/javascript">
        ...
        angular.module("myApp").controller('SummaryController', ['$scope', '$http', function ($scope, $http) {
            ...
            $scope.destroy = function(){
                $http.get('/booking/destroy/admin').success(function(d){
                    if(d.result){
                        alert('จบการทำงาน บัญชี : {{ session()->get('member')->name }}');
                        window.close();
                    }
                });
            }
